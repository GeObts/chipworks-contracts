> **PROVENANCE — added when this file was copied into the contracts repo, 2026-09-03.**
> Everything below the rule is Base's own documentation, verbatim, fetched via
> `https://docs.base.org/llms.txt` during the site build session. It is filed here unedited
> so the B20 reconciliation in `ASSUMPTIONS.md` (A-13, A-15, A-18) has its source sitting
> next to it rather than in another repo.
>
> **What it settles, and where that is recorded:**
> - Balances do **not** rebase on a corporate action — "without changing their balance of the
>   B20 token". Closes OPEN_ITEMS item 14.
> - The Chainlink feeds are **total-return**: they publish `underlying price x multiplier`,
>   which is the TOKEN price. Confirms `StockRegistry.priceUsd` needs no multiplier maths.
> - **All thirteen** tickers have a published feed, not nine. Corrects A-13 and A-18.
> - The feed **freezes during corporate actions**, and the docs say plainly: *"never settle
>   or liquidate against a frozen feed."* Directly endorses `ChipRounds.maxFeedAge`.
> - Heartbeat is **24h during market hours**, with no off-hours updates at all.
> - "Tokens should be identified by address rather than ticker or symbol. Metadata is mutable
>   onchain." Endorses `test/b20/IdentifyByAddress.t.sol`.
>
> Nothing below has been altered. Base's docs are living; treat this as a dated snapshot.
...
```text Token price formula theme={null}
