...
///        - **Fuel (Chiplets):** `ERC721Burnable`, so {forge} calls `burn(tokenId)` on tokens
///          the user has approved it for. That emits `Transfer(owner, 0, tokenId)` and
///          **decrements `totalSupply`** — the collection visibly shrinks. See {_consumeFuel}.
...
contract Furnace is Ownable2Step, ReentrancyGuard, IERC721Receiver {
    ...
    ///      explorers. Chiplets exposes `burn`, so at launch these should track each other.
    ...
    ///
    ///      APPROVE-THEN-BURN, AND WHY THAT IS NOT A RUG VECTOR. Chiplets is
    ///      `ERC721Burnable`, whose `burn` authorises the caller exactly the way `transferFrom`
    ///      does — owner, approved-for-token, or operator. The Furnace holds no burn ROLE and
    ///      cannot be granted one; it can only reach tokens a user has approved it for, in the
    ///      ordinary marketplace way (`setApprovalForAll(furnace, true)`).
    ///
