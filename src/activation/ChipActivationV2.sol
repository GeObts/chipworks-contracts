...
///         operation. Where that $CHIP lands is {chipBurnTarget}: pointed at {ChipBurner}
///         (as it is in production) the supply genuinely falls, because $CHIP's `burn` is a
///         standard PUBLIC burn of the caller's own balance. Pointed at `0xdead` the tokens
///         are merely unreachable and `totalSupply` does not move. Earlier notes here said
///         $CHIP had no `burn()` at all and that `0xdead` was therefore the only option —
///         that is wrong about the deployed token. Corrected 2026-09-10.
///
...
contract ChipActivationV2 is IActivationSource, Ownable2Step, ReentrancyGuard {
    ...
    ///        - **The $CHIP is destroyed too, via {chipBurnTarget}.** An earlier version of
    ///          this note claimed the opposite — that Bankr's Doppler token "exposes no
    ///          `burn`" so the $CHIP could only ever sit at `0xdead`. That is wrong about the
    ///          DEPLOYED token: `burn(uint256)` is public and burns the caller's own balance.
    ///          In production {chipBurnTarget} is {ChipBurner}, which holds the stream and
    ///          calls that `burn`, so `totalSupply` really falls. Corrected 2026-09-10.
    ///
    ///          ONE TIMING DIFFERENCE WORTH KEEPING STRAIGHT, and the reason
    ///          {effectiveChipSupply} still exists: the Chiplet stops existing in THIS
    ///          transaction, whereas the $CHIP leaves supply only when someone calls
    ///          `ChipBurner.burnAll()`. Between the two it is held, not yet burned. The
    ///          keeper runs `burnAll` every cycle (`chipworks-keeper`, tasks/burn.ts).
    ///          See {effectiveChipSupply} and BURN_VISIBILITY.md.
    ///
