# Audit brief, ROUND 5 (pre-mainnet): ChipWorks Box (Box + PrizeVault + ChipConverter)

**Chain:** Base mainnet (8453). **Solidity:** 0.8.24, EVM `cancun`, OpenZeppelin 5.1.0, optimizer 200 runs, no `via_ir`.
**Source commit:** `e150d61` in `chipworks-contracts`, branch `box-rebuild`. Every file under `contracts/`, `tests/`, `gas/` and `reference/` is byte-identical to that commit after CRLF→LF. The brief and the `TRIAGE-*.md` files are pack documents: they are first committed together with the pack, one commit later.
**Round 5, the last before mainnet.** Round 4 reviewed `a73fa4d`. What changed since then is in §R5 and `TRIAGE-ROUND4.md` (paste 3), which also records the **owner's decisions** on every open item. Earlier rounds: §R4 + `TRIAGE-ROUND3.md`, §R3 + `TRIAGE-ROUND2.md`, §R + `TRIAGE-ROUND1.md`.
**Please review the WHOLE contract set, with extra attention on §R5.** Toolchain for every number here: **Foundry 1.7.1**, solc 0.8.24.
**Pastes:** 1 = this brief + `Box.sol` · 2 = `PrizeVault`, `ChipConverter` and every interface (1 and 2 are the priority) · 3 = the triage of rounds 3, 2 and 1 with each round's verification tests · 4 = audit PoCs, the Base fork test and both live-node gas simulations · 5 = the rebuild and vault tests and the shared fixture · 6 = the remaining unit tests and the mocks · 7 = reference (optional: the v4 swap pattern the converter follows, and Pyth's own `revealWithCallback`).
...
## R5. What changed since round 4 (read with TRIAGE-ROUND4.md)

- **Launch odds ("option B", owner-chosen).** `Box._loadLaunchOdds` sets these tiers:
  ...
  Weights sum to 10,000, and RTP is 9,100 bps = 91.00%.
  - The roll bands are [0,3500) [3500,8500) [8500,9450) [9450,9850) [9850,9950) [9950,10000).
  - The top tier is unchanged at 36×, so every size's jackpot, and the reserve it needs, is unchanged.
- **Box sizes.** `MAX_SKUS` goes from 3 to **8**.
  - The constructor sets five sizes: $1 / $10 / $25 **on sale**, and $50 (id 3) and $100 (id 4) **existing but paused** (`SkuPaused` emitted).
  - Slots 5–7 can be added later through `queueSku` (48h).
  - **The reserve interaction is deliberate.** `maxLivePrizeUsd` counts only sizes that exist and are unpaused, plus `maxSoldPrizeUsd`. So a paused, never-sold $100 size doesn't hold back `sweepSurplus` / `executeSurplusWithdraw` behind a $14,400 reserve.
  - Once unpaused (`setSkuPaused`, immediate), a size counts in the reserve, and the sell gate opens it once `prizeCapUsd` ≥ its jackpot: $1,800 at a $7,200 pool, $3,600 at $14,400.
  - The loop in `maxLivePrizeUsd` now covers 8 slots. It runs only in the sweep and the withdraw, never in a buy or the callback.
  - Box is **23,843 bytes**.
- **Owner decisions** (TRIAGE-ROUND4): BOX-R4-L1, BOX-L5, the BOX-L3 residual, BOX-L2 and the H-3 residual are **accepted and documented** (§8, items 8–13). No write-off function. The ChipLottery redeploy is deferred.
- **Round-4 test nits:**
  - The cold callback test now runs at `MIN_CALLBACK_GAS` and asserts which stock paid.
  - Both sims use the new table's tier-2 rolls; `VIEM_FROM` override added.
  - Two older tests had stopped exercising the thin-stock and blacklisted-stock skips after the start became hash-picked. They are fixed and now assert the start index.
- **New tests:** `test/box/BoxLaunchConfig.t.sol` covers the exact table and RTP, and shows $50/$100 unlocking at exactly $7,200/$14,400 (one micro-dollar short still reverts `SkuNotCovered`).
- **Please answer Q18–Q20 (§9) as well as Q1–Q17.**

## R4. What changed from round 3 to round 4 (read with TRIAGE-ROUND3.md)

- **BOX-R3-L1:** `IPrizeVault.settle(to, prizeUsd, entropy, gasBounded)`. Box passes `true` from the Pyth
  callback and `false` from `claimOwed`. Only the callback stops the stock walk after
  `MAX_FAILED_TRANSFERS = 2`. `claimOwed` is an ordinary transaction, so it walks every stock and then USDC.
  A box sent OWED by two refusals is therefore paid by the next stock that accepts. This is deliberately
  not `gasleft()`-gated: on the `revealWithCallback` recovery path the caller chooses the gas. Box: **23,801 bytes**.
- **Real-node worst case** (`gas/box-refusing-opener-sim.cjs` + `.out.txt`, paste 4):
  - Setup: an OFAC-listed opener (real B20 policies and Circle both refuse it), 13 listed stocks, 10 funded by real restocks.
  - The callback uses **580,918 gas**: 2 real refused transfers, USDC refused, then OWED.
  - At `MAX_STOCKS = 16` that is about 680k, by the measured 33.1k/stock slope.
- **Tests:**
  - scenario H, and `claimOwed` walking every refusing stock;
  - a cold callback delivered with exactly `callbackGasLimit` gas;
  - the mock pool manager now checks the price-limit direction (X1).
- **NatSpec:** `minUsdcBps >= maxPrizeBps` is described as holding at restock and sweep time, not always.

## R3. What changed from round 2 to round 3 (read with TRIAGE-ROUND2.md)
...
- **BOX-L1:** `Box.setChipPaused(bool)`, owner, immediate. It stops both `buyWithChip` paths without touching USDC sales.
- **BOX-L6:** `PrizeVault.MAX_FAILED_TRANSFERS = 2`. After two refused stock transfers, the callback's payout walk goes to USDC (in round 4: the callback only).
...
- **Still open, as owner decisions:** BOX-L2 (immediate vault setters), BOX-L5 (the registry owner is the same Safe), the BOX-L3 residual, the H-3 residual, and M-restock.
...
Sealed, giftable ERC-721 "gacha" boxes. A buyer pays **$1, $10 or $25** (and later **$50 / $100**, paused at launch; 8 size slots) in **USDC or $CHIP** for a
...
**Contracts (pastes 1 and 2):** `src/box/Box.sol` (ERC-721, buy/open/callback/owed prizes), `src/box/PrizeVault.sol`
...
price whatever the market does. The pool fees are the buyer's (**~2.3–2.4%** over the mark on the
Base fork for a $10 box; market-dependent; the UI discloses it). The swap code is the deployed ChipLottery's
(`reference/ChipLottery.sol`, paste 7), with the USDC recipient changed.
...
output in paste 4). B20 stocks are node precompiles that cannot run in a forge fork, so this is the
only real measurement:

| stocks listed in the vault (tx `gasUsed`, incl. ~21.8k intrinsic) | callback gas, pays first stock | callback gas, skips all others (EMPTY) first |
|---|---|---|
| 10 (all currently enabled) | 467,147 | 465,018 |
| 13 (all registered) | 566,543 | 574,176 |
| 13 listed, 10 funded, **opener refused by every B20 and USDC** (`box-refusing-opener-sim.cjs`) | — | **580,918** tx (≈559k execution; Grok: min callee cap 567,577, ok at 900k) (2 real refusals, then OWED) |

The cost is **pricing the pool** (`PrizeVault._snapshot`: ~22.5k per registry mark, ~33.1k per listed
unfunded stock and ~33.9k per funded one in total), not the B20 transfer (a B20 `balanceOf` is 2.6k). The first build priced every stock
twice per payout and measured **529k against a 500k limit**. It now prices once, and:

- `PrizeVault.MAX_STOCKS = 16` is a **hard gas bound**: the refusing-opener worst case is ~680k at 16 by the measured slope (13 are registered today, so 16 cannot be measured). Stocks can
...
- **Cap.** One prize ≤ `maxPrizeBps` (25%) of `inventoryUsd()` (USDC + enabled, fresh-marked stocks).
  Every change to the cap, up or down, is 48h-timelocked; ceiling 50%, and never above `minUsdcBps`.
...
48h timelock, 14-day grace: fee recipient (`queueTreasury`), SKU price/existence, odds table, every change to
`maxPrizeBps`, up or down (never above the USDC share), vault surplus withdraw. The StockRegistry (feeds, venues) is owned by the same Safe, with no timelock (BOX-L5). Once only: `vault.setBox`,
...
6. **Buys are gas-heavy** (665k with 10 stocks, 764k with 13) because the sell gate values the whole pool. Accepted on Base.
7. **Box.sol is 23,843 bytes** (limit 24,576; our budget 24,000). Proposed fixes to Box must fit in ~150 bytes,
   or move logic out of Box.

**Owner-accepted (2026-09-26; the Safe is a trusted 2-of-3 operator; the pool stays solvent in each case).**
Tell us if you think any of these is rated too low. Please don't re-report them unless you find a NEW
way to exploit them.

8. **BOX-R4-L1: an owed prize to an address every asset refuses (sanctioned) is never paid.** The box
   stays OWED and 110% of its prize stays reserved in USDC. There is no write-off, by design (legal).
   Keepers `eth_call` `claimOwed` before sending.
9. **BOX-L5: the StockRegistry owner is the same Safe, with no timelock** on feeds and venues. The Safe is the trust root.
10. **BOX-L3: `maxFeedAge` defaults to 5 days** (ceiling 7). It can accept a weekday corporate-action
    freeze. The feeds are total-return, so a freeze causes no price jump. Anything under ~4 days breaks long weekends.
11. **BOX-L2: PrizeVault setters are immediate** (`setKeeper`, `setRestockParams`, `setMaxFeedAge`,
    `setStockEnabled`). All are bounded, and the floors hold.
12. **H-3 residual:** a disabled or stale stock is invisible to the withdraw's reserve check. The withdraw
    is 48h-timelocked, and the floors hold on what remains.
13. **M-restock:** `restock` ignores the 110% floor. The leak is at most slippage (≤5%) × the daily cap.

**Q18 (round 5).** Box sizes. With 8 slots, 5 set at launch and $50/$100 paused:
- Can any path make a paused or never-sold size shrink or inflate the reserve wrongly?
- Can it let the sweep take the pool below what an UNPAUSED size needs, or strand a sold box?
- Can `setSkuPaused` / `queueSku` on a big size be used to drain surplus that sold boxes need?

**Q19 (round 5).** The launch odds. Check:
- the table, the roll bands and `rtpBps() == 9_100`;
- the `_drawAt` boundaries at 3499/3500, 8499/8500, 9449/9450, 9849/9850 and 9949/9950;
- that `maxPrizeUsd` / `maxSoldPrizeUsd` use 36× for every size.

**Q20 (round 5).** Anything in the full contract set that all earlier rounds missed.
...
(needs `BASE_RPC_URL`); `node tools/box/box-callback-sim.cjs`, `STOCKS=all node tools/box/box-callback-sim.cjs` and `node tools/box/box-refusing-opener-sim.cjs`
