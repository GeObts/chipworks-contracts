# Audit brief, ROUND 4: ChipWorks Box (Box + PrizeVault + ChipConverter)

**Chain:** Base mainnet (8453). **Solidity:** 0.8.24, EVM `cancun`, OpenZeppelin 5.1.0, optimizer 200 runs, no `via_ir`.
**Source commit:** `a73fa4d` in `chipworks-contracts`, branch `box-rebuild`. Every file under `contracts/`, `tests/`, `gas/` and `reference/` is byte-identical to that commit after CRLF→LF. The brief and the `TRIAGE-*.md` files are pack documents: they are first committed together with the pack, one commit later.
**Round 4.** Round 3 reviewed `81c7e2e` (its `src/` = `2943917`). What changed since then, and why, is in §R4 below and `TRIAGE-ROUND3.md` (paste 3). Earlier rounds: §R3 + `TRIAGE-ROUND2.md`, §R + `TRIAGE-ROUND1.md`.
**Please focus on the round-4 diff (§R4)**, then anything the earlier rounds missed.
...
## R4. What changed since round 3 (read with TRIAGE-ROUND3.md)
...
| 13 listed, 10 funded, **opener refused by every B20 and USDC** (`box-refusing-opener-sim.cjs`) | — | **580,918** (2 real refusals, then OWED) |
...
7. **Box.sol is 23,801 bytes** (limit 24,576; our budget 24,000). Proposed fixes to Box must fit in ~200 bytes,
