# Audit brief, ROUND 2: ChipWorks Box (Box + PrizeVault + ChipConverter)

**Chain:** Base mainnet (8453). **Solidity:** 0.8.24, EVM `cancun`, OpenZeppelin 5.1.0, optimizer 200 runs, no `via_ir`.
**Source commit:** `fdb77c4` in `chipworks-contracts`, branch `box-rebuild`. The packaged files are byte-identical to that commit after CRLF→LF.
**Round 2.** Round 1 reviewed `c98ee57`; what changed and why is in §R below and `TRIAGE-ROUND1.md` (paste 3).
...
## R. What changed since round 1 (read with TRIAGE-ROUND1.md)
...
`maxPrizeBps`, vault surplus withdraw. Once only: `vault.setBox`,
...
7. **Box.sol is 23,596 bytes** (limit 24,576; our budget 24,000). Proposed fixes to Box must fit in ~400 bytes,
